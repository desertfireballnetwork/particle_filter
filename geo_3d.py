...
#
#    36,    37,    38,       39,       40,       41
# X_eci, Y_ECI, Z_ECI, X_DT_ECI, Y_DT_ECI, Z_DT_ECI
#------------------------------------------------------------------------------
...
def Initialise(x0, v0, index, oindex, N, P, params, alpha, date_info, mass_opt=3, m0_max=2000., gamma= 0.7854, eci_bool=False):
    ...
    X[3] = random.gauss(v0[0], v0[0] * P[3]) # initial velocity in x using Gaussian pdf. P values are already sqrt(cov)
    X[4] = random.gauss(v0[1], v0[1] * P[4]) # initial velocity in y using Gaussian pdf. P values are already sqrt(cov)
    X[5] = random.gauss(v0[2], v0[2] * P[5]) # initial velocity in z using Gaussian pdf. P values are already sqrt(cov)
    ...
    #X[7] = random.gauss(cd_mean, cd_std)
...
def Prdct_Upd(X, mu, tkm1, tk, fireball_info, obs_info, lum_info, index, N, frag, t_end, Q_c, m0_max, reverse=False, eci_bool=False):
    ...
    ##TODO check this!!
    # if there is a numerical anomaly caused by masses going close to 0, leave as is and return with a weight ~0.
    # if fireball_info[2]<55000.:
    #     if X[6]>init_x[6] or norm([X[3], X[4], X[5]]) > norm([init_x[3], init_x[4], init_x[5]]):
    #         print('mass increasing... removing particle', init_x[6] , X[6], norm([init_x[3], init_x[4], init_x[5]]), norm([X[3], X[4], X[5]]))
    #         X[0:13] = init_x
    #         X[29] = -500.
    #         return X
    ...
    X[29], X[34] = Get_Weighting(X, obs_info, lum_info, N, t_end, m0_max,reverse, eci_bool)
...
def Get_Weighting(X, obs_info, lum_info, N, t_end, m0_max,reverse=False, eci_bool=False):
    ...
    #     print('mass above 110%')
        ...
        # print(z_hat.T, Z.T, cov, n_obs, pos_weight)
