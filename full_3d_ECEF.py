...
def Prdct_Upd(X, mu, tkm1, tk, fireball_info, obs_info, lum_info, index, N, frag, t_end, Q_c, m0_max, reverse=False, eci_bool=False):
    ...
    Q_d = bf.Q_mx_3d(tkm1, tk,  init_x, mu, po, grav, Qc)
    ...
    X[29], X[34] = Get_Weighting(X, obs_info, lum_info, N, t_end, m0_max,reverse)
    ...
    pos, vel = tu.ECEF2ECI(np.vstack((X[0], X[1], X[2])), 
...
def Get_Weighting(X, obs_info, lum_info, N, t_end, m0_max, reverse):
